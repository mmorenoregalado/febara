...
| `ai`              | `stream` (`POST /ai/stream`), `identifyPokemonCard` (`POST /ai/identify-pokemon-card`) | Ver sección 5 |
...
```ts
export const textModel = openai("gpt-4o-mini");        // chat / streaming de texto
...
export const identifyCardModel = google(gemini-2.5-flash-lite); // visión multimodal
```
...
3. **Llamada al LLM**: usa `generateText` del AI SDK con `identifyCardModel` (Gemini), pasando un mensaje
   ...
   ```ts
   CardIdentificationSchema = { identified: boolean, pokemonName: string|null,
...
Usuario sube foto (PokemonCardUpload.vue)
   ...
   → generateText({ model: identifyCardModel(Gemini), Output.object(CardIdentificationSchema) })
