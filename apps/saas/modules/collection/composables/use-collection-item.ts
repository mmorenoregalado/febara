...
export const useCollectionMutations = (pokemonId: MaybeRefOrGetter<number>) => {
	const { orpc } = useORPC();
	...
	const invalidateCollection = async () => {
		...
		await queryClient.invalidateQueries({ queryKey: orpc.collection.containsMany.key() });
	};
...
export const useCollectionItem = (pokemonId: MaybeRefOrGetter<number>) => {
	...
	const { add, remove, isAdding, isRemoving } = useCollectionMutations(pokemonId);
